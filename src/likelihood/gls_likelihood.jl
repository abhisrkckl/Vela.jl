...
function calc_noise_weights_inv(kernel::WoodburyKernel, params::NamedTuple)
    return vcat(
        (
            Float32.(calc_noise_weights_inv(gp_comp, params)) for
...
function _calc_resids_and_Ninvdiag(
    ...
    result_data = Vector{Float32}(undef, 2 * ntoas)
...
function _calc_resids_and_Ninvdiag(
    ...
    ntoas = length(wtoas)
    result_data = Vector{Float32}(undef, 4 * ntoas)
    ...
    if parallel
        @inbounds @threads for j = 1:ntoas
            ...
            Ninvdiag[j] = value(1.0 / scaled_toa_error_sqr(wtoa.toa, cwtoa.toa_correction))
            Ninvdiag[ntoas+j] =
                value(1.0 / scaled_dm_error_sqr(wtoa.dminfo, cwtoa.dm_correction))
        end # COV_EXCL_LINE
    else
...
function calc_lnlike_vectorized(
    ...
    result = Vector{Float32}(undef, nparamss)
