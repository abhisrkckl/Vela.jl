export FrequencyDependentBase, FrequencyDependent, FrequencyDependentJump

"""
    FrequencyDependentBase

An abstract base type representing frequency-dependent delays to account for 
frequency-dependent profile evolution.
"""
abstract type FrequencyDependentBase <: DelayComponent end

"""
    FrequencyDependent

A frequency-dependent delay to account for frequency-dependent profile evolution. 
Corresponds to `FD` in `PINT`.

Reference:
    [Arzoumanian+ 2015](http://doi.org/10.1088/0004-637X/813/1/65)
"""
struct FrequencyDependent <: FrequencyDependentBase end
...
A system- & frequency-dependent delay to account for frequency-dependent profile evolution.
This is useful while combining TOAs generated by different experiments with different TOA measurement
procedures. Corresponds to `FDJump` in `PINT`.

Reference:
    [Susobhanan+ 2024](http://doi.org/10.3847/1538-4357/ad59f7)
"""
struct FrequencyDependentJump <: FrequencyDependentBase
    jump_mask::BitMatrix
